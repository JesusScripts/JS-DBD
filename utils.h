...
#include <cmath>
#include <iostream>
#include "logger.h" // Include a logger header
...
static POINT WorldToScreen(FMinimalViewInfo camera, FVector WorldLocation) {
    ...
    auto start = std::chrono::high_resolution_clock::now(); // Start time for logging
    ...
    auto end = std::chrono::high_resolution_clock::now(); // End time for logging
    std::chrono::duration<double> duration = end - start;
    logger::log("WorldToScreen processing time: " + std::to_string(duration.count()) + " seconds");
...
static std::string GetNameById(uint32_t actor_id)
{
    ...
    if (sLength < 128)
    {
        read_array<char>(TableLocationAddress + 6, pNameBuffer, sLength);
        return std::string(pNameBuffer, sLength);
    }
